...
export function redactMemory(c:string):{redacted:string,safe:boolean}{let r=c;for(const{p:pat,r:repl}of PATTERNS){r=r.replace(pat,repl)}return{redacted:r,safe:r===c}}
